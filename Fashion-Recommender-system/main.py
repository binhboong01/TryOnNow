...
def save_new_cloth(uploaded_file):
    try:
        with open(os.path.join("newData", uploaded_file.name), 'wb') as f:
            f.write(uploaded_file.getbuffer())
            return 1
    except:
        return 0
...
uploaded_new_cloth = st.file_uploader("Upload image data")
if uploaded_new_cloth is not None:
    if save_new_cloth(uploaded_new_cloth):
        st.write("File uploaded")
        show_images = Image.open(uploaded_new_cloth)
        size = (400, 400)
        resized_im = show_images.resize(size)
        st.image(resized_im)
        # extract features of uploaded image
        features = extract_img_features(os.path.join("newData", uploaded_new_cloth.name), model)
        features_list.append(features)
        img_files_list.append(os.path.join("newData", uploaded_new_cloth.name))
        pickle.dump(features_list, open("image_features_embedding.pkl", "wb"))
        pickle.dump(img_files_list, open("img_files.pkl", "wb"))
    else:
